...
int32_t FastMulInnerBase(BigNum_stt *P_pBN, uint16_t P_exp);

/* x=x>>(esp words).  Right shift of esp words. *******************************/
void FastDivInnerBase(BigNum_stt * P_pBN, uint16_t P_exp);

/* x=x<<shift. Left shift of 'shift' bits. Condition: 0<shift<32. *************/
int32_t LeftShift(BigNum_stt * P_pBN, uint32_t P_shift);

void RightShift(BigNum_stt *P_pBN, uint32_t P_shift);

/* x=x*y. *********************************************************************/
int32_t MulBigbyUnInt(BigNum_stt *P_pBN,uint32_t P_factor2);
