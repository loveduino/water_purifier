...
/*! \page Tutorial_DES_CBC DES-CBC Tutorial
  *
  * Here follows an example of DES-CBC encryption.
  ...
  *  - \ref DES_CBC_Encrypt_Init initialize an \ref DESCBCctx_stt context with key and initialization vector
  *  - \ref DES_CBC_Encrypt_Append process the input and produces the output. 
  *    It can be called multiple times but the input size must be multiple of 8. 
  *  - \ref DES_CBC_Encrypt_Finish can be called only one time for the finalization process
  *
  * The following code performs a CBC encryption with DES of 1024 in 4 Append calls.
  * \code
  * #include <stdio.h>
  * #include "crypto.h"
  * int32_t main()
  ...
  *   DESctx_st.mIvSize=8;
  *   // call init function
  *   retval = DES_CBC_Encrypt_Init(&DESctx_st, key, iv);
  ...
  *     retval = DES_CBC_Encrypt_Append(&DESctx_st, plaintext, 256, ciphertext, &outSize);
  *     if (retval != DES_SUCCESS)
  *     {  ... }
  *   }
  *   //Do the finalization call (in CBC it will not return any output)
  *   retval = DES_CBC_Encrypt_Finish(&DESctx_st, ciphertext+outSize, &outSize );
  *   if (retval != DES_SUCCESS)
  *   {  ... }
  * }
  * \endcode
...
int32_t DES_CBC_Encrypt_Init (DESCBCctx_stt *P_pDESCBCctx, const uint8_t *P_pKey, const uint8_t *P_pIv);
...
/* load the key and ivec, eventually performs key schedule, init hw, etc. *****/
int32_t DES_CBC_Decrypt_Init (DESCBCctx_stt *P_pDESCBCctx, const uint8_t *P_pKey, const uint8_t *P_pIv);
