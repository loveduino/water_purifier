/**
  ...
  * You may obtain a copy of the License at:h
  *
  ...
  for (session_done = 0, errors = 0, session_begin = 0; ;)
  {
    for (packets_received = 0, file_done = 0, buf_ptr = buf; ;)
    {
      switch (Receive_Packet(packet_data, &packet_length, NAK_TIMEOUT))
      {
              ...
              else
              {
                if (packets_received == 0)//第一包数据不写入到flash中
                {
                  ...
                  if (packet_data[PACKET_HEADER] != 0)
                  {
                    ...
                    //判斷file name是否符合要求
#if APP_A
                    if (NULL == strstr((char*)FileName, "app_b.bin"))
#elif APP_B | BOOTLOADER
                    if (NULL == strstr((char*)FileName, "app_a.bin"))
#endif
                    {
                        /* End session */
                        Send_Byte(CA);
        ...
        case 1://
          Send_Byte(CA);/* two of these in succession aborts transfer */
          Send_Byte(CA);/* two of these in succession aborts transfer */
          return -3;
          
        default://接受超时或者序列不对或者CRC错误
          if (session_begin > 0)
          {
            errors ++;
          }
