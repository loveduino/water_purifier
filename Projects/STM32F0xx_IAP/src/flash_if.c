...
uint32_t FLASH_If_DisableWriteProtection(uint32_t application_address)
{
  ...
  uint32_t flash_protected_pages = 0;
  if (APPLICATION_ADDRESS_A == application_address)
  {
      flash_protected_pages = FLASH_PROTECTED_PAGES_A;
  }
  else if (APPLICATION_ADDRESS_B == application_address)
  {
      flash_protected_pages = FLASH_PROTECTED_PAGES_B;
  }
  ...
  if (FLASH_If_GetWriteProtectionStatus(application_address) != 0x00)
  {
...
/**
  ...
  *         value is reset.
  */
uint32_t FLASH_If_GetWriteProtectionStatus(uint32_t application_address)
{ 
  uint32_t flash_protected_pages = 0;
  if (APPLICATION_ADDRESS_A == application_address)
  {
